        ...
        <div class="add-form">
          <el-dialog title="新增套餐"
            ...
            <div slot="footer"
              ...
              <el-button type="primary"
                         @click="handleAdd('addSetMealForm')">确定</el-button>
            </div>
          </el-dialog>
        </div>
        <!-- 编辑标签弹层 -->
        <div class="add-form">
          <el-dialog title="编辑套餐"
            ...
            <div slot="footer"
              ...
              <el-button type="primary"
                         @click="handleEdit('editSetMealForm')">确定</el-button>
            </div>
          </el-dialog>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      // 查询对应的检查组
      this.$http
        .get('api/setmeal/getPicAndCheckGroupIdsBySetmealId/' + row.id)
        .then((res) => {
          if (res.data.flag) {
            // 查询ok
            ...
            this.imageUrl =
              'https://linjundey.oss-cn-shanghai.aliyuncs.com/pic/' +
    ...
    handleAvatarSuccess(res, file) {
      this.imageUrl =
        'https://linjundey.oss-cn-shanghai.aliyuncs.com/pic/' + res.data
      this.formData.img = res.data // 将图片的uuid插入数据库
    },
    // 文件上传前检查
      ...
      this.$refs[validForm].validate((valid) => {
        if (valid) {
          if (this.checkgroupIds.length == 0) {
            this.$message.error('请至少勾选一个检查组')
            this.activeName = 'second'
            return
          }
          this.formData.checkgroupIds = this.checkgroupIds
          this.$http
            .post('api/setmeal/addSetmeal', this.formData)
            .then((res) => {
              if (res.data.flag) {
                // 添加成功
